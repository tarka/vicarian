...
fn test_router() -> Result<()> {
    ...
    assert_eq!(Uri::from_static("http://localhost:1010"), *backend_url(&matched.backend));

    let matched = router.lookup("/base/path").unwrap();
    assert_eq!(Uri::from_static("http://localhost:1010"), *backend_url(&matched.backend));

    let matched = router.lookup("/service").unwrap();
    assert_eq!(Uri::from_static("http://localhost:2020"), *backend_url(&matched.backend));
    ...
    assert_eq!(Uri::from_static("http://localhost:3030"), *backend_url(&matched.backend));
    ...
    assert_eq!(Uri::from_static("http://localhost:4040"), *backend_url(&matched.backend));
...
fn test_router_prefix_ambiguity() -> Result<()> {
    ...
    assert_eq!(Uri::from_static("http://localhost:9999"), *backend_url(&matched.backend));
...
fn test_router_many_backends() -> Result<()> {
    ...
    assert_eq!(1000, backend_url(&m.backend).port_u16().unwrap());

    let m = router.lookup("/svc050").unwrap();
    assert_eq!(1050, backend_url(&m.backend).port_u16().unwrap());

    let m = router.lookup("/svc099/deep/path").unwrap();
    assert_eq!(1099, backend_url(&m.backend).port_u16().unwrap());

    // Non-matching falls to root
    let m = router.lookup("/svc100").unwrap();
    assert_eq!(9999, backend_url(&m.backend).port_u16().unwrap());
...
fn test_router_reverse_input_order() -> Result<()> {
    ...
    assert_eq!(Uri::from_static("http://localhost:1010"), *backend_url(&m.backend));

    let m = router.lookup("/m/x").unwrap();
    assert_eq!(Uri::from_static("http://localhost:2020"), *backend_url(&m.backend));

    let m = router.lookup("/z/x").unwrap();
    assert_eq!(Uri::from_static("http://localhost:3030"), *backend_url(&m.backend));
